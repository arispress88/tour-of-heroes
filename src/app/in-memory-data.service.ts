...
export class InMemoryDataService implements InMemoryDbService {
  ...
  genId(heroes: Hero[]): number {
    return heroes.length > 0 ? Math.max(...heroes.map(hero => hero.id)) + 1 : 11; // the spread operator (...) is a JavaScript syntax for adding the array elements into a list of arguments
  }
}
